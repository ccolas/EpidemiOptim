...
class World2(BaseModel):
    ...
    def define_params_and_initial_state_distributions(self):
        ...
        self._all_initial_state_distribs = dict(P0=DiracDist(params=1.65e9, stochastic=self.stochastic),
                                                NR0=DiracDist(params=900e9, stochastic=self.stochastic),
                                                CI0=DiracDist(params=0.4e9, stochastic=self.stochastic),
                                                POL0=DiracDist(params=0.2e9, stochastic=self.stochastic),
                                                CIAF0=DiracDist(params=0.2, stochastic=self.stochastic),
                                                BR0=None,
                                                DR0=None,
                                                CR0=None,
                                                NRUR0=DiracDist(params=0., stochastic=self.stochastic),
                                                NRFR0=None,
                                                CIR0=None,
                                                CIG0=None,
                                                CID0=None,
    ...
    def _sample_initial_state(self):
        ...
        for k in self._all_initial_state_distribs.keys():
            if self._all_initial_state_distribs[k] != None:
                self.initial_state[k[:-1]] = self._all_initial_state_distribs[k].sample()
            else:
                self.initial_state[k[:-1]] = None  # to complete with formulas below
        ...
        self.initial_state['POLG'] = self.initial_state['P'] * self.f['POLN'](self.year_min) * self.f['POLCM'](self.initial_state['CIR'])
        ...
        self.initial_state['FR'] = (self.f['FPCI'](self.initial_state['CIRA']) * self.f['FCM'](self.initial_state['CR'])) * \
                                   (self.f['FPM'](self.initial_state['POLR']) * self.f['FC'](self.year_min)) / self.initial_internal_params['FN']
    ...
    def update_mode_params(self, new_params):
        assert sorted(self.current_internal_params.keys()) == sorted(new_params.keys())
        self.current_internal_params = new_params
        self.update_funcs()


    def run_one_step(self, state, time):

        new_state = dict()
        new_time = time + self.dt
        # update population state variable
        new_state['BR'] = state['P'] * self.f['BRN'](time) * self.f['BRMM'](state['MSL']) * self.f['BRCM'](state['CR']) * \
                          self.f['BRFM'](state['FR']) * self.f['BRPM'](state['POLR'])
        new_state['DR'] = state['P'] * self.f['DRN'](time) * self.f['DRMM'](state['MSL']) * self.f['DRPM'](state['POLR']) * \
        ...
        new_state['NRUR'] = (state['P'] * self.f['NRUN'](time)) * self.f['NRMM'](state['MSL'])
        ...
        new_state['CID'] = state['CI'] * self.f['CIDN'](time)
        new_state['CIG'] = (state['P'] * self.f['CIM'](state['MSL'])) * self.f['CIGN'](time)
        ...
        new_state['POLG'] = state['P'] * self.f['POLN'](time) * self.f['POLCM'](state['CIR'])
        ...
        new_state['FR'] = (self.f['FCM'](new_state['CR']) * self.f['FPCI'](new_state['CIRA']) * self.f['FPM'](new_state['POLR']) * self.f['FC'](new_time)) / \
    ...
    def run_n_steps(self, current_state=None, n=1, labelled_states=False, time=None):
        ...
        for i in range(n):
            new_state, time = self.run_one_step(states[-1], time)
